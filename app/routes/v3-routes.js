...
router.post('/v3/sign-in-password', function (req, res) {
  ...
  if (req.session.data['signin-password'] === '') {
    ...
    // User inputted value so move to next page
    res.redirect('/v3/ch-verified')
  }
})


// ******* ch-verified javascript ********************************
router.get('/v3/ch-verified', function (req, res) {
  // Set URl
  res.render('v3/ch-verified', {
    currentUrl: req.originalUrl
  })
})

router.post('/v3/ch-verified', function (req, res) {
  // Create empty array
  var errors = []
...
router.get('/v3/how-verify', function (req, res) {
  // Set URl
  res.render('v3/how-verify', {
    currentUrl: req.originalUrl
  })
})

router.post('/v3/how-verify', function (req, res) {
  // Create empty array
  var errors = []
  ...
  if (typeof req.session.data['how-did-you-verify'] === 'undefined') {
    ...
    if (req.session.data['how-did-you-verify'] === 'onelogin') {
      res.redirect('/v3/sign-in-verified')
    }
    else {
      res.redirect('/v3/uvid')
    }
  }
})

// ******* how-verify javascript ********************************
router.get('/v3/dob', function (req, res) {
  // Set URl
  res.render('v3/dob', {
    currentUrl: req.originalUrl
  })
...
router.post('/v3/dob', function (req, res) {
    // Create empty array and set error variables to false
    var errors = []
  ...
  if (dobError) {
    ...
    if (req.session.data['uvid'] === 'xxxx') {
      res.redirect('/v3/acsp-not-verified')
    }
    else {
      res.redirect('/v3/acsp-verified')
    }
  }
})
...
// ******* uvid validation ********************************
router.get('/v3/uvid', function (req, res) {
  // Set URl
  res.render('v3/uvid', {
    currentUrl: req.originalUrl
  })
...
router.post('/v3/uvid', function (req, res) {
  // Create empty array and set error variables to false
  var errors = []
...
// ******* sign-in-verified ********************************
router.get('/v3/sign-in-verified', function (req, res) {
  // Set URl
  res.render('v3/sign-in-verified', {
    currentUrl: req.originalUrl
  })
})

router.post('/v3/sign-in-verified', function (req, res) {
  res.redirect('/v3/verify-create-or-sign-in')
})


// ******* verified-email ********************************
router.get('/v3/verified-email', function (req, res) {
  // Set URl
  res.render('v3/verified-email', {
    currentUrl: req.originalUrl
  })
})

router.post('/v3/verified-email', function (req, res) {
  // Create empty array and set error variables to false
  var errors = []
  ...
  if (req.session.data['verified-email'] === '') {
    ...
    // User inputted value so move to next page
    res.redirect('/v3/verified-password')
  }
})


// ******* verified-password ********************************
router.get('/v3/verified-password', function (req, res) {
  // Set URl
  res.render('v3/verified-password', {
    currentUrl: req.originalUrl
  })
})

router.post('/v3/verified-password', function (req, res) {
  // Create empty array and set error variables to false
  var errors = []
